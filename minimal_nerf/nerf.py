...
class NeRFNetwork(nn.Module):
    ...
    def add_model_argparse_args(parent_parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
        ...
        parser.add_argument("--depth", help="number of layers from position to the feature vector and density layer",type=int)
        parser.add_argument("--width", help="width of the layers from the position to the feature vector and density layer",type=int)
        parser.add_argument("--use_skip_connection",help="Use a skip connection halfway through the layers between position and density/feature vector", action=argparse.BooleanOptionalAction)
...
class NeRF(pl.LightningModule):
    ...
    def add_model_argparse_args(parent_parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
        ...
        parser.add_argument("--n_coarse_samples", help="number of coarse samples to take for each ray that needs to be rendered. More will result in more accurate renders but results in slower rendering.",type=int)
        parser.add_argument("--min_render_distance", help="minimal rendering distance for each ray. important to configre this correctly",type=float)
        parser.add_argument("--max_render_distance", help="maximal rendering distance for each ray. Important to configure this correctly.",type=float)
        parser.add_argument("--debug",help="log debug information such as model gradients and rendering steps ", action=argparse.BooleanOptionalAction)
        
        parser = NeRFNetwork.add_model_argparse_args(parser)
        
        return parent_parser
    def __init__(self, min_render_distance:float = 2.0, max_render_distance:float = 6.0, n_coarse_samples:int = 64,debug:bool=False, **kwargs) -> None:
        self.debug = debug
    ...
    def render_rays(self,ray_origins: torch.Tensor, ray_directions: torch.Tensor, n_coarse_samples:int) -> torch.Tensor:
    ...
    def validation_step(self, batch: torch.Tensor, batch_idx):
        ...
        if isinstance(self.logger, WandbLogger):
            self.logger.log_image(key=f"train_{batch_idx}", images =[image,rendered_image], caption=["train","rendered"])
